...
impl Default for LndRest {
    fn default() -> Self {
        Self {
            base_url: Url::parse("http://localhost:8080s").unwrap(),
...
impl LndRest {
    pub fn new(base_url: &str, macaroon_hex: String) -> Result<Self, Error> {
    ...
    pub fn create_lnd_address(&self, lnd: &Lnd, options: &Options) -> Result<String, Error> {
        let url = self.build_url("/v1/newaddress?type=p2tr");
        let result: Value = self.client.get(url).send()?.json()?;
