use crate::{
    copy_file, get_absolute_path, ImageInfo, L1Node, L2Node, LndCli, LndRest, NodeCommand,
...
impl Lnd {
    ...
    pub fn get_peers_channel_point(
        ...
        if let Some(rest) = self.lnd_rest.clone() {
            rest.get_peers_channel_point(self, options, node_command)
        } else {
            self.lnd_cli
...
impl L2Node for Lnd {
    ...
    fn get_node_pubkey(&self, options: &Options) -> Result<String, Error> {
        if let Some(rest) = self.lnd_rest.clone() {
            rest.get_node_pubkey(self, options)
        } else {
    ...
    fn pay_invoice(
        ...
        if let Some(rest) = self.lnd_rest.clone() {
            rest.pay_invoice(self, options, node_command, payment_request)
        } else {
            self.lnd_cli
    ...
    fn create_on_chain_address(&self, options: &Options) -> Result<String, Error> {
        if let Some(rest) = self.lnd_rest.clone() {
            rest.create_lnd_address(self, options)
        } else {
    ...
    fn pay_address(
        ...
        if let Some(rest) = self.lnd_rest.clone() {
            rest.pay_address(self, options, node_command, address)
        } else {
            self.lnd_cli
    ...
    fn get_preimage(&self, options: &Options, rhash: String) -> Result<String, Error> {
        if let Some(rest) = self.lnd_rest.clone() {
            rest.get_preimage(self, options, rhash)
        } else {
    ...
    fn create_hold_invoice(
        ...
        if let Some(rest) = self.lnd_rest.clone() {
            rest.create_hold_invoice(self, option, node_command, rhash)
        } else {
            self.lnd_cli
    ...
    fn settle_hold_invoice(&self, options: &Options, preimage: String) -> Result<(), Error> {
        if let Some(rest) = self.lnd_rest.clone() {
            rest.settle_hold_invoice(self, options, &preimage)
        } else {
